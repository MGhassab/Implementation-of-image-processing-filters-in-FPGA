--------------------------------------------------------
-- Image procesing Average filter
...
architecture average_beh of average is
  ...
  signal out_avg_ram                : ram_type;                             -- output Ram for save output 
  signal data_out_1                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  signal data_out_2                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  signal data_out_3                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  signal data_out_4                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  signal data_out_5                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  signal data_out_6                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  signal data_out_7                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  signal data_out_8                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  signal data_out_9                 : std_logic_vector(15 downto 0) ;       -- use for compute filter output . GET pixel value form input RAM
  ...
  signal nine : std_logic_vector(15 downto 0) := "0000000000001001";  -- make constant 9 for divider
begin
   -- registers
  process (clk,rst)
  begin
    if (rst = '1')  then                    -- determine reset values
      ...
      out_avg_ram(to_integer(unsigned(add_outram_reg))) <= data_outram(7 downto 0);  -- write new data on OUT_RAM
      
        data_out_1 <= "00000000" & in_ram(to_integer((unsigned(row_up_reg)*unsigned(column))+unsigned(counter_reg)-1));   -- UP_LEFT house
        data_out_2 <= "00000000" & in_ram(to_integer((unsigned(row_up_reg)*unsigned(column))+unsigned(counter_reg)));     -- UP_CENTER house
        data_out_3 <= "00000000" & in_ram(to_integer((unsigned(row_up_reg)*unsigned(column))+unsigned(counter_reg)+1));   -- UP_RIGHT house
        
        data_out_4 <= "00000000" & in_ram(to_integer((unsigned(row_center_reg)*unsigned(column))+unsigned(counter_reg)-1)); -- CENTER_LEFT house
        data_out_5 <= "00000000" & in_ram(to_integer((unsigned(row_center_reg)*unsigned(column))+unsigned(counter_reg)));   -- CENTER house
        data_out_6 <= "00000000" & in_ram(to_integer((unsigned(row_center_reg)*unsigned(column))+unsigned(counter_reg)+1)); -- CENTER_RIGHT house
        
        data_out_7 <= "00000000" & in_ram(to_integer((unsigned(row_down_reg)*unsigned(column))+unsigned(counter_reg)-1)); -- DOWN_LEFT house
        data_out_8 <= "00000000" & in_ram(to_integer((unsigned(row_down_reg)*unsigned(column))+unsigned(counter_reg)));   -- DOWN_CENTER house
        data_out_9 <= "00000000" & in_ram(to_integer((unsigned(row_down_reg)*unsigned(column))+unsigned(counter_reg)+1)); -- DOWN_RIGHT house
    ...
    process(state_reg,start,input,column,row,row_cnt_reg,add_inram_reg,counter_reg,row_down_reg,row_center_reg,row_up_reg,add_outram_reg)
    begin
      case state_reg is       -- determine select
       ...
       when   proces    =>                            -- 3)PROCES state : compute and generate filter OUT and GET new data from input port
           if(row_cnt_reg = std_logic_vector(unsigned(row) + 2)) then   -- for determin end of process
              ...
              if (counter_reg < std_logic_vector(unsigned(column)-1)) then           -- for go to column-1 pixel 
                      ...
                      data_outram <= std_logic_vector(((unsigned(data_out_1)+unsigned(data_out_2)+unsigned(data_out_3)+unsigned(data_out_4)+unsigned(data_out_5)+unsigned(data_out_6))+unsigned(data_out_7)+unsigned(data_out_8)+unsigned(data_out_9))/unsigned(nine));
              ...
              if(add_inram_reg = std_logic_vector((unsigned(column)+unsigned(column)+unsigned(column)+unsigned(column))-1)) then           -- address IN_RAM = 4xcolumn - 1     
                  data_in <= input;                     
                  add_inram_next <= (others => '0');    -- becaude we have 4 row(0 to 3) and the memory read and write is circular we should set zero address
